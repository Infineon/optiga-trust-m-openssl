...
#define TRUSTM_PROVIDER_ERRFN(x, ...) \
    fprintf( \
        stderr, \
        "%d:Error in %s:%d %s: " x "\n", \
        getpid(), \
...
#define TRUSTM_PROVIDER_MSGFN(x, ...) \
    fprintf( \
        stderr, \
        "%d:Message:%s:%d %s: " x "\n", \
        getpid(), \
...
#define TRUSTM_ERROR_raise_text(core, reason, ...) \
    (trustm_new_error((core), (reason), __VA_ARGS__), TRUSTM_ERROR_set_debug(core))
#define TRUSTM_ERROR_set_debug(core) \
    trustm_set_error_debug((core), OPENSSL_FILE, OPENSSL_LINE, OPENSSL_FUNC)
//

#define TRUSTM_MAX_OSSL_NAME 50

// OSSL error enum with TRUSTM
enum { TRUSTM_ERR_INIT_FAILURE = 1, TRUSTM_ERR_CANNOT_GET_RANDOM };
