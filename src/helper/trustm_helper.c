...
optiga_lib_status_t trustmReadMetadata(uint16_t optiga_oid, trustm_metadata_t *oidMetadata) {
    ...
    do {
        ...
        optiga_lib_status = OPTIGA_LIB_BUSY;
        return_status =
            optiga_util_read_metadata(me_util, optiga_oid, read_data_buffer, &bytes_to_read);
        if (OPTIGA_LIB_SUCCESS != return_status)
            break;
...
optiga_lib_status_t trustmProviderReadMetadata(
    optiga_util_t *me_util,
    uint16_t optiga_oid,
...
optiga_lib_status_t trustm_readUID(utrustm_UID_t *UID) {
    ...
    do {
        ...
        optiga_lib_status = OPTIGA_LIB_BUSY;
        return_status =
            optiga_util_read_data(me_util, optiga_oid, offset, read_data_buffer, &bytes_to_read);
