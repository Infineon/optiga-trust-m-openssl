...
static void *trustm_digest_newctx(void *provctx) {
    ...
    (trustm_digest_ctx->data->hash_context).context_buffer =
        trustm_digest_ctx->data->hash_context_buffer;
    (trustm_digest_ctx->data->hash_context).context_buffer_length =
        sizeof(trustm_digest_ctx->data->hash_context_buffer);
...
static void *trustm_digest_dupctx(void *ctx) {
    ...
    memcpy(
        dstctx->data->hash_context_buffer,
        src->data->hash_context_buffer,
...
static int trustm_digest_update(void *ctx, const unsigned char *in, size_t inl) {
    ...
    if (trustm_digest_ctx->data->hash_context.context_buffer[0] == 0) {
        ...
        if (optiga_lib_status != OPTIGA_LIB_SUCCESS) {
            TRUSTM_PROVIDER_ERRFN(
                "Error in Hash context reinitializing\nError code : 0x%.4X\n",
                return_status
            );
            goto error;
        }
...
static int trustm_digest_final(void *ctx, unsigned char *out, size_t *outl, size_t outsz) {
    ...
    if (OPTIGA_LIB_SUCCESS != return_status) {
        TRUSTM_PROVIDER_ERRFN(
            "Error in optiga_crypt_hash_final\nError code : 0x%.4X\n",
            return_status
        );
        goto error;
    }
