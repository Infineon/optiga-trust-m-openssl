...
ASN1_SEQUENCE(TRUSTM_RSA_PUBKEY) =
    {ASN1_SIMPLE(TRUSTM_RSA_PUBKEY, n, ASN1_INTEGER),
     ASN1_SIMPLE(TRUSTM_RSA_PUBKEY, e, ASN1_INTEGER)} ASN1_SEQUENCE_END(TRUSTM_RSA_PUBKEY)
...
static TRUSTM_RSA_PUBKEY *trustm_get_rsa_pubkey(const trustm_rsa_key_t *pkey) {
    ...
    if (pkey->key_size == OPTIGA_RSA_KEY_1024_BIT_EXPONENTIAL) {
        modulus_length =
            pkey->public_key
                [pkey->public_key_header_length + 9];  // get the total length in bytes of modulus
        memcpy(
            modulus_buffer,
            (pkey->public_key + pkey->public_key_header_length + 10),
            modulus_length
        );
    } else {
        modulus_length = (pkey->public_key[pkey->public_key_header_length + 11]) << 8
                         | (pkey->public_key[pkey->public_key_header_length + 12]
                         );  // get the total length in bytes of modulus

        memcpy(
            modulus_buffer,
            (pkey->public_key + pkey->public_key_header_length + 13),
            modulus_length
        );
    }
...
static int trustm_rsa_encoder_encode_text(
    void *ctx,
    OSSL_CORE_BIO *cout,
    const void *key,
...
static int trustm_rsa_encode_public_SubjectPublicKeyInfo_pem(
    trustm_encoder_ctx_t *trustm_encoder_ctx,
    BIO *bout,
...
static int trustm_rsa_encoder_encode_SubjectPublicKeyInfo_pem(
    void *ctx,
    OSSL_CORE_BIO *cout,
    const void *key,
    const OSSL_PARAM key_abstract[],
    ...
    if (selection & OSSL_KEYMGMT_SELECT_PUBLIC_KEY)
        ret = trustm_rsa_encode_public_SubjectPublicKeyInfo_pem(
            trustm_encoder_ctx,
            bout,
            trustm_rsa_key
        );
...
static OSSL_FUNC_encoder_does_selection_fn
    trustm_rsa_encoder_SubjectPublicKeyInfo_pem_does_selection;
...
static int trustm_rsa_encoder_encode_SubjectPublicKeyInfo_der(
    void *ctx,
    OSSL_CORE_BIO *cout,
    const void *key,
    const OSSL_PARAM key_abstract[],
    ...
    if (selection & OSSL_KEYMGMT_SELECT_PUBLIC_KEY)
        ret = trustm_rsa_encode_public_SubjectPublicKeyInfo_der(
            trustm_encoder_ctx,
            bout,
            trustm_rsa_key
        );
...
static OSSL_FUNC_encoder_does_selection_fn
    trustm_rsa_encoder_SubjectPublicKeyInfo_der_does_selection;
...
static int trustm_rsa_encoder_encode_pkcs1_pem(
    void *ctx,
    OSSL_CORE_BIO *cout,
    const void *key,
    const OSSL_PARAM key_abstract[],
...
///////////////////////////////////////////////////////////////////////////////////////
static int trustm_rsa_encode_public_pkcs1_der(
    trustm_encoder_ctx_t *trustm_encoder_ctx,
    BIO *bout,
    trustm_rsa_key_t *trustm_rsa_key
) {
...
static int trustm_rsa_encoder_encode_pkcs1_der(
    void *ctx,
    OSSL_CORE_BIO *cout,
    const void *key,
    const OSSL_PARAM key_abstract[],
...
static X509_PUBKEY *trustm_get_x509_ec_pubkey(trustm_ec_key_t *pkey) {
    ...
    if (!X509_PUBKEY_set0_param(
            ...
            V_ASN1_OBJECT,
            OBJ_nid2obj(trustm_ecc_curve_to_nid(pkey->key_curve)),
            penc,
            penclen
        )) {
        OPENSSL_free(penc);
...
/////////////////////////////////////////////////////////////////////////////////////////////////
static int trustm_ec_encode_public_SubjectPublicKeyInfo_pem(
    trustm_encoder_ctx_t *trustm_encoder_ctx,
    BIO *bout,
...
static int trustm_ec_encoder_encode_SubjectPublicKeyInfo_pem(
    void *ctx,
    OSSL_CORE_BIO *cout,
    const void *key,
    const OSSL_PARAM key_abstract[],
    ...
    if (selection & OSSL_KEYMGMT_SELECT_PUBLIC_KEY)
        ret = trustm_ec_encode_public_SubjectPublicKeyInfo_pem(
            trustm_encoder_ctx,
            bout,
            trustm_ec_key
        );
...
static OSSL_FUNC_encoder_does_selection_fn
    trustm_ec_encoder_SubjectPublicKeyInfo_pem_does_selection;
...
static int trustm_ec_encode_public_SubjectPublicKeyInfo_der(
    trustm_encoder_ctx_t *trustm_encoder_ctx,
    BIO *bout,
    trustm_ec_key_t *trustm_ec_key
) {
...
static int trustm_ec_encoder_encode_SubjectPublicKeyInfo_der(
    void *ctx,
    OSSL_CORE_BIO *cout,
    const void *key,
    const OSSL_PARAM key_abstract[],
    ...
    if (selection & OSSL_KEYMGMT_SELECT_PUBLIC_KEY)
        ret = trustm_ec_encode_public_SubjectPublicKeyInfo_der(
            trustm_encoder_ctx,
            bout,
            trustm_ec_key
        );
...
static OSSL_FUNC_encoder_does_selection_fn
    trustm_ec_encoder_SubjectPublicKeyInfo_der_does_selection;
...
static int trustm_ec_encoder_encode_text(
    void *ctx,
    OSSL_CORE_BIO *cout,
    const void *key,
    const OSSL_PARAM key_abstract[],
...
static int trustm_ec_encode_PrivateKeyInfo_pem(
    void *ctx,
    OSSL_CORE_BIO *cout,
    const void *key,
    const OSSL_PARAM key_abstract[],
...
static int trustm_rsa_encode_PrivateKeyInfo_pem(
    void *ctx,
    OSSL_CORE_BIO *cout,
    const void *key,
    const OSSL_PARAM key_abstract[],
...
static int trustm_rsa_encode_PrivateKeyInfo_der(
    void *ctx,
    OSSL_CORE_BIO *cout,
    const void *key,
    const OSSL_PARAM key_abstract[],
