...
int trustm_ecc_public_key_to_point(trustm_ec_key_t *trustm_ec_key) {
    ...
    if (trustm_ec_key->key_curve == OPTIGA_ECC_CURVE_NIST_P_256
        ...
        uncompressed_buff_length =
            trustm_ec_key->public_key[trustm_ec_key->public_key_header_length + 1] - 1;
        memcpy(
            uncompressed_buff,
            (trustm_ec_key->public_key + trustm_ec_key->public_key_header_length + 3),
            uncompressed_buff_length
        );
    }
        ...
        uncompressed_buff_length =
            trustm_ec_key->public_key[trustm_ec_key->public_key_header_length + 2] - 1;
        memcpy(
            uncompressed_buff,
            (trustm_ec_key->public_key + trustm_ec_key->public_key_header_length + 4),
            uncompressed_buff_length
        );
    }

    if ((group = EC_GROUP_new_by_curve_name(trustm_ecc_curve_to_nid(trustm_ec_key->key_curve)))
            == NULL
...
int trustm_buffer_to_ecc_point(
    ...
    if ((group = EC_GROUP_new_by_curve_name(trustm_ecc_curve_to_nid(trustm_ec_key->key_curve)))
            == NULL
        || (point = EC_POINT_new(group)) == NULL
        || !EC_POINT_oct2point(group, point, buf, len, NULL) || (x = BN_new()) == NULL
        || (y = BN_new()) == NULL || !EC_POINT_get_affine_coordinates(group, point, x, y, NULL))
        goto final;
...
der_append_tlv(unsigned char *p, unsigned char tag, const unsigned char *val, size_t vlen) {
    *p++ = tag;
