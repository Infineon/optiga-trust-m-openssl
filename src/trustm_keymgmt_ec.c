...
static int trustm_ec_keymgmt_gen_set_params(void *ctx, const OSSL_PARAM params[]) {
    ...
    if (keyId_str == NULL) {
        ...
        if ((trustm_ec_gen_ctx->private_key_id < 0xE0F0)
            || (trustm_ec_gen_ctx->private_key_id > 0xE0F3)) {
...
static void *trustm_ec_keymgmt_gen(void *ctx, OSSL_CALLBACK *cb, void *cbarg) {
    ...
    TRUSTM_PROVIDER_DBG(
        "Key OID: 0x%.4X Key curve: 0x%.2X Key usage: 0x%.2X\n",
    ...
    uint16_t public_id = ((trustm_ec_key->key_curve == OPTIGA_ECC_CURVE_NIST_P_521)
                          || (trustm_ec_key->key_curve == OPTIGA_ECC_CURVE_BRAIN_POOL_P_512R1))
                             ? (trustm_ec_key->private_key_id + 0x10EF)
                             : (trustm_ec_key->private_key_id + 0x10E0);
    ...
    if (OPTIGA_LIB_SUCCESS != return_status) {
        TRUSTM_PROVIDER_ERRFN(
            "Error in optiga_util_write_data\nError code : 0x%.4X\n",
            return_status
        );
        goto error;
    }
...
static int trustm_ec_keymgmt_get_params(void *keydata, OSSL_PARAM params[]) {
    ...
    if (p != NULL
        && !OSSL_PARAM_set_utf8_string(
            p,
            OBJ_nid2sn(trustm_ecc_curve_to_nid(trustm_ec_key->key_curve))
        ))
        return 0;
...
static int trustm_ec_keymgmt_has(const void *keydata, int selection) {
    ...
    if ((selection & OSSL_KEYMGMT_SELECT_PRIVATE_KEY) != 0
        ...
        res = res && (trustm_ec_key->point_x_buffer_length > 0)
              && (trustm_ec_key->point_y_buffer_length > 0);
    TRUSTM_PROVIDER_DBGFN("<");
    return res;
}
