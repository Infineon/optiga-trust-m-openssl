...
static int trustm_rsa_keymgmt_gen_set_params(void *ctx, const OSSL_PARAM params[]) {
    ...
    if (trustm_rsa_gen_ctx->private_key_id < 0xE0FC
        || trustm_rsa_gen_ctx->private_key_id > 0xE0FD) {
...
static void *trustm_rsa_keymgmt_gen(void *ctx, OSSL_CALLBACK *cb, void *cbarg) {
    ...
    if (OPTIGA_LIB_SUCCESS != return_status) {
        TRUSTM_PROVIDER_ERRFN(
            "Error in optiga_crypt_rsa_generate_keypair\nError code : 0x%.4X\n",
            return_status
        );
        goto error;
    }
    ...
    if (return_status != OPTIGA_LIB_SUCCESS) {
        TRUSTM_PROVIDER_ERRFN(
            "Error writing public key to OID 0x%.4X\n",
            (trustm_rsa_key->private_key_id) + 0x10E4
        );
        goto error;
    }
    ...
    printf(
        "Writing public key to OID 0x%.4X  SUCCESS\n",
        (trustm_rsa_key->private_key_id) + 0x10E4
    );
    ...
    if (selection & OSSL_KEYMGMT_SELECT_PUBLIC_KEY) {
        ...
        if (reversed_modulus) {
            for (size_t i = 0; i < trustm_rsa_key->modulus_length; i++) {
                reversed_modulus[i] =
                    trustm_rsa_key->modulus[trustm_rsa_key->modulus_length - 1 - i];
            }
...
static int trustm_rsa_keymgmt_import(void *keydata, int selection, const OSSL_PARAM params[]) {
    ...
    if (selection & OSSL_KEYMGMT_SELECT_PRIVATE_KEY) {
        ...
        if (p != NULL) {
            ...
            TRUSTM_PROVIDER_DBGFN(
                "Imported private_key_id: 0x%04x",
                trustm_rsa_key->private_key_id
            );
        }
    }
