...
static void *rsa_asymcipher_newctx(void *provctx) {
    ...
    trustm_rsa_asymcipher_ctx_t *trustm_rsa_asymcipher_ctx =
        OPENSSL_zalloc(sizeof(trustm_rsa_asymcipher_ctx_t));
    ...
    trustm_rsa_asymcipher_ctx->encrypted_message_length =
        sizeof(trustm_rsa_asymcipher_ctx->encrypted_message);
    trustm_rsa_asymcipher_ctx->decrypted_message_length =
        sizeof(trustm_rsa_asymcipher_ctx->decrypted_message);
    TRUSTM_PROVIDER_DBGFN("<");
...
static int rsa_asymcipher_encrypt(
    void *ctx,
    unsigned char *out,
    size_t *outlen,
    size_t outsize,
    ...
    return_status = optiga_crypt_rsa_encrypt_message(
        trustm_rsa_asymcipher_ctx->me_crypt,
    ...
    if (OPTIGA_LIB_SUCCESS != return_status) {
        TRUSTM_PROVIDER_ERRFN(
            "Error in optiga_crypt_rsa_encrypt_message\nError code : 0x%.4X\n",
            return_status
        );
        goto error;
    }
    ...
    *outlen = trustm_rsa_asymcipher_ctx->encrypted_message_length;
    ...
    if (out != NULL) {
        if (*outlen > outsize) {
            TRUSTM_PROVIDER_ERRFN(
                "Error outlen : %d is larger than outsize : %d\n",
                *outlen,
                outsize
            );
            goto error;
...
static int rsa_asymcipher_decrypt(
    void *ctx,
    unsigned char *out,
    size_t *outlen,
    size_t outsize,
    ...
    if (OPTIGA_LIB_SUCCESS != return_status) {
        TRUSTM_PROVIDER_ERRFN(
            "Error in optiga_crypt_rsa_decrypt_and_export\nError code : %.4X\n",
            return_status
        );
        goto error;
    }
    ...
    if (return_status != OPTIGA_LIB_SUCCESS) {
        TRUSTM_PROVIDER_ERRFN(
            "Error decrypting message with RSA.\nError code : %.4X\n",
            return_status
        );
        goto error;
    }
