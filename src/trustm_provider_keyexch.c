...
static int trustm_keyexch_set_peer(void *ctx, void *provkey) {
    ...
    uncompressed_peerkey_buffer_length =
        trustm_ec_point_to_uncompressed_buffer(peerkey, (void **)&uncompressed_peerkey_buffer);
    ...
    if (peerkey->key_curve == OPTIGA_ECC_CURVE_NIST_P_521
        ...
        memcpy(
            trustm_keyexch_ctx->peer_buffer + 4,
            uncompressed_peerkey_buffer,
            uncompressed_peerkey_buffer_length
        );
        ...
        // copy uncompressed form into buffer
        memcpy(
            trustm_keyexch_ctx->peer_buffer + 3,
            uncompressed_peerkey_buffer,
            uncompressed_peerkey_buffer_length
        );
...
static int trustm_keyexch_derive_kdf(
    trustm_keyexch_ctx_t *trustm_keyexch_ctx,
    ...
    if (trustm_keyexch_ctx->kdf_outlen > outlen) {
        TRUSTM_PROVIDER_ERRFN(
            "Error kdf_outlen : %d  larger than outlen : %d\n",
            trustm_keyexch_ctx->kdf_outlen,
            outlen
        );
        return 0;
    }
    ...
    return_status = optiga_crypt_ecdh(
        trustm_keyexch_ctx->me_crypt,
    ...
    if (return_status != OPTIGA_LIB_SUCCESS) {
        TRUSTM_PROVIDER_ERRFN(
            "Error in trustm_keyexch_derive_kdf\nError code : 0x%.4X\n",
            return_status
        );
        goto error;
    }

    if ((kdf = EVP_KDF_fetch(
             trustm_keyexch_ctx->libctx,
             trustm_keyexch_ctx->kdf_name,
             trustm_keyexch_ctx->kdf_propq
         )) == NULL
    ...
    *p++ =
        OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_KEY, shared_secret, shared_secret_length);
    *p++ = OSSL_PARAM_construct_octet_string(
        OSSL_KDF_PARAM_INFO,
        trustm_keyexch_ctx->kdf_ukmptr,
        trustm_keyexch_ctx->kdf_ukmlen
    );
...
static int trustm_keyexch_derive_plain(
    trustm_keyexch_ctx_t *trustm_keyexch_ctx,
    unsigned char *secret,
    size_t *secretlen,
    size_t outlen
    ...
    if (return_status != OPTIGA_LIB_SUCCESS) {
        TRUSTM_PROVIDER_ERRFN(
            "Error in trustm_keyexch_derive_plain\nError code : 0x%.4X\n",
            return_status
        );
        goto error;
    }
    ...
    *secretlen = shared_secret_length;
    if (secret != NULL) {
        if (*secretlen > outlen) {
            TRUSTM_PROVIDER_ERRFN(
                "Error secretlen  :  %d   larger than outlen  :  %d\n",
                *secretlen,
                outlen
            );
            goto error;
...
static int
trustm_keyexch_derive(void *ctx, unsigned char *secret, size_t *secretlen, size_t outlen) {
    trustm_keyexch_ctx_t *trustm_keyexch_ctx = ctx;
