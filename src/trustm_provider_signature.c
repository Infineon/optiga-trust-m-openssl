...
static void *trustm_signature_newctx(void *provctx, const char *proq) {
    ...
    (trustm_signature_ctx->digest_data->hash_context).context_buffer =
        trustm_signature_ctx->digest_data->hash_context_buffer;
    (trustm_signature_ctx->digest_data->hash_context).context_buffer_length =
        sizeof(trustm_signature_ctx->digest_data->hash_context_buffer);
...
static void *trustm_signature_dupctx(void *ctx) {
    ...
    memcpy(
        sctx->digest_data->hash_context_buffer,
        src->digest_data->hash_context_buffer,
    ...
    (sctx->digest_data->hash_context).context_buffer_length =
        sizeof(sctx->digest_data->hash_context_buffer);
...
static int
rsa_signature_scheme_init(trustm_signature_ctx_t *trustm_signature_ctx, const char *mdname) {
    if (mdname == NULL) {
...
static int
ecdsa_signature_scheme_init(trustm_signature_ctx_t *trustm_signature_ctx, const char *mdname) {
    if (mdname == NULL) {
...
static int trustm_rsa_signature_sign_init(void *ctx, void *provkey, const OSSL_PARAM params[]) {
    ...
    return (
        rsa_signature_scheme_init(trustm_signature_ctx, NULL)
...
static int trustm_ecdsa_signature_sign_init(void *ctx, void *provkey, const OSSL_PARAM params[]) {
    ...
    return (
        ecdsa_signature_scheme_init(trustm_signature_ctx, NULL)
        && trustm_ecdsa_signature_set_ctx_params(trustm_signature_ctx, params)
    );
}

static int trustm_rsa_signature_sign(
    void *ctx,
    unsigned char *sig,
    size_t *siglen,
    size_t sigsize,
    ...
    if (return_status != OPTIGA_LIB_SUCCESS) {
        TRUSTM_PROVIDER_ERRFN(
            "Error signing in trustm_rsa_signature_sign\nError code : 0x%.4X\n",
            return_status
        );
        goto error;
    }
    ...
    *siglen = temp_siglen;
    if (sig != NULL) {
        if (*siglen > sigsize) {
            TRUSTM_PROVIDER_ERRFN(
                "Error output siglen : %d larger than sigsize : %d\n",
                *siglen,
                sigsize
            );
            goto error;
...
static int trustm_ecdsa_signature_sign(
    void *ctx,
    unsigned char *sig,
    size_t *siglen,
    size_t sigsize,
    ...
    if (OPTIGA_LIB_SUCCESS != return_status) {
        TRUSTM_PROVIDER_ERRFN(
            "Error signing in optiga_crypt_ecdsa_sign\nError code : 0x%.4X\n",
            return_status
        );
        goto error;
    }
...
static int trustm_rsa_signature_digest_init(
    void *ctx,
    const char *mdname,
    void *provkey,
    ...
    trustm_crypt_ShieldedConnection();
    optiga_lib_status = OPTIGA_LIB_BUSY;
    return_status = optiga_crypt_hash_start(
        trustm_signature_ctx->me_crypt,
    ...
    if (OPTIGA_LIB_SUCCESS != return_status) {
        TRUSTM_PROVIDER_ERRFN(
            "Error in optiga_crypt_hash_start\nError code : 0x%.4X\n",
            return_status
        );
        goto error;
    }
...
static int trustm_ecdsa_signature_digest_init(
    void *ctx,
    const char *mdname,
    void *provkey,
    const OSSL_PARAM params[]
...
static int
trustm_rsa_signature_digest_update(void *ctx, const unsigned char *data, size_t datalen) {
    trustm_signature_ctx_t *trustm_signature_ctx = ctx;
    optiga_lib_status_t return_status;
    ...
    trustm_crypt_ShieldedConnection();
    optiga_lib_status = OPTIGA_LIB_BUSY;
    return_status = optiga_crypt_hash_update(
        trustm_signature_ctx->me_crypt,
        &(trustm_signature_ctx->digest_data->hash_context),
    ...
    if (OPTIGA_LIB_SUCCESS != return_status) {
        TRUSTM_PROVIDER_ERRFN(
            "Error in optiga_crypt_hash_update\nError code : 0x%.4X\n",
            return_status
        );
        goto error;
    }
    ...
    trustm_crypt_ShieldedConnection();
    optiga_lib_status = OPTIGA_LIB_BUSY;
    return_status = optiga_crypt_hash_finalize(
        trustm_signature_ctx->me_crypt,
        &(trustm_signature_ctx->digest_data->hash_context),
    ...
    if (OPTIGA_LIB_SUCCESS != return_status) {
        TRUSTM_PROVIDER_ERRFN(
            "Error in optiga_crypt_hash_finalize\nError code : 0x%.4X\n",
            return_status
        );
        goto error;
    }
...
static int
trustm_ecdsa_signature_digest_update(void *ctx, const unsigned char *data, size_t datalen) {
    trustm_signature_ctx_t *trustm_signature_ctx = ctx;
    optiga_lib_status_t return_status;
...
static int trustm_rsa_signature_digest_sign_final(
    void *ctx,
    unsigned char *sig,
    size_t *siglen,
    size_t sigsize
    ...
    if (OPTIGA_LIB_SUCCESS != return_status) {
        TRUSTM_PROVIDER_ERRFN(
            "Error in optiga_crypt_rsa_sign\nError code : 0x%.4X\n",
            return_status
        );
        goto error;
    }
...
static int trustm_ecdsa_signature_digest_sign_final(
    void *ctx,
    unsigned char *sig,
    size_t *siglen,
    size_t sigsize
    ...
    if (OPTIGA_LIB_SUCCESS != return_status) {
        TRUSTM_PROVIDER_ERRFN(
            "Error in optiga_crypt_ecdsa_sign\nError code : 0x%.4X\n",
            return_status
        );
        goto error;
    }
...
static int trustm_rsa_signature_digest_sign(
    void *ctx,
    unsigned char *sig,
    size_t *siglen,
    size_t sigsize,
...
static int trustm_ecdsa_signature_digest_sign(
    void *ctx,
    unsigned char *sig,
    size_t *siglen,
    size_t sigsize,
...
static int
trustm_rsa_signature_digest_verify_final(void *ctx, const unsigned char *sig, size_t siglen) {
    trustm_signature_ctx_t *trustm_signature_ctx = ctx;
    optiga_lib_status_t return_status;
    uint8_t digest_size;
    ...
    if (trustm_signature_ctx->trustm_rsa_key->key_size == OPTIGA_RSA_KEY_2048_BIT_EXPONENTIAL) {
        ...
        if (trustm_signature_ctx->trustm_rsa_key->modulus[0] > 0x7F) {
            ...
            memcpy(
                public_key_buffer + 14,
                trustm_signature_ctx->trustm_rsa_key->modulus,
                trustm_signature_ctx->trustm_rsa_key->modulus_length
            );
            ...
            public_key_buffer_length +=
                1 + trustm_signature_ctx->trustm_rsa_key->modulus_length + 5;
        }

        else {
            memcpy(
                public_key_buffer + 13,
                trustm_signature_ctx->trustm_rsa_key->modulus,
                trustm_signature_ctx->trustm_rsa_key->modulus_length
            );
    ...
    if (OPTIGA_LIB_SUCCESS != return_status) {
        TRUSTM_PROVIDER_ERRFN(
            "Error verifying in optiga_crypt_rsa_verify\n Error code: 0x%.4x\n",
            return_status
        );
        goto error;
    }
    ...
    if (return_status != OPTIGA_LIB_SUCCESS) {
        TRUSTM_PROVIDER_ERRFN(
            "Error verifying in trustm_rsa_signature_digest_verify_final\nError code: 0x%.4x\n",
            return_status
        );
        goto error;
    }
...
static int
trustm_ecdsa_signature_digest_verify_final(void *ctx, const unsigned char *sig, size_t siglen) {
    trustm_signature_ctx_t *trustm_signature_ctx = ctx;
    optiga_lib_status_t return_status;
    uint8_t digest_size;
    public_key_from_host_t public_key_details;
    ...
    uncompressed_pubkey_buffer_length = trustm_ec_point_to_uncompressed_buffer(
        trustm_signature_ctx->trustm_ec_key,
        &uncompressed_pubkey_buffer
    );
    ...
    if (OPTIGA_LIB_SUCCESS != return_status) {
        TRUSTM_PROVIDER_ERRFN(
            "Error verifying in optiga_crypt_ecdsa_verify\n Error code: 0x%.4x\n",
            return_status
        );
        goto error;
    }
    ...
    if (return_status != OPTIGA_LIB_SUCCESS) {
        TRUSTM_PROVIDER_ERRFN(
            "Error verifying in trustm_ecdsa_signature_digest_verify_final\nError code: 0x%.4x\n",
            return_status
        );
        goto error;
    }
...
const OSSL_DISPATCH trustm_rsa_signature_functions[] = {
    ...
    {OSSL_FUNC_SIGNATURE_DIGEST_VERIFY_FINAL,
     (void (*)(void))trustm_rsa_signature_digest_verify_final},
