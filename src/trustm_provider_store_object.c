...
static void *trustm_object_open(void *provctx, const char *uri) {
    ...
    if (((key_id < 0xE0F0) || (key_id > 0xE0F3)) && ((key_id < 0xE0FC) || (key_id > 0xE0FD))) {
...
static int trustm_genpkey_rsa(trustm_object_ctx_t *trustm_object_ctx) {
    ...
    if (return_status != OPTIGA_LIB_SUCCESS) {
        TRUSTM_PROVIDER_ERRFN(
            "Error in RSA public key saving\n Return status: %d\n",
            return_status
        );
        goto error;
    }
...
static int trustm_object_load_pkey_rsa(
    trustm_object_ctx_t *trustm_object_ctx,
    ...
    return_status = trustmProviderReadMetadata(
        trustm_object_ctx->me_util,
        trustm_object_ctx->key_id,
        &oidMetadata
    );
    if (OPTIGA_LIB_SUCCESS != return_status) {
        TRUSTM_PROVIDER_ERRFN(
            "Error reading metadata of OID: 0x%.4X\nError code: 0x%.4X\n",
    ...
    if (return_status != OPTIGA_LIB_SUCCESS) {
        TRUSTM_PROVIDER_ERRFN(
            "Error reading contents of OID: 0x%.4X\n",
            trustm_rsa_key->private_key_id + 0x10E4
        );
        goto error;
    }
    ...
    if (trustm_rsa_key->key_size == OPTIGA_RSA_KEY_1024_BIT_EXPONENTIAL) {
        modulus_length = trustm_rsa_key->public_key
                             [trustm_rsa_key->public_key_header_length
                              + 9];  // get the total length in bytes of modulus
        memcpy(
            modulus_buffer,
            (trustm_rsa_key->public_key + trustm_rsa_key->public_key_header_length + 10),
            modulus_length
        );
    } else {
        modulus_length =
            (trustm_rsa_key->public_key[trustm_rsa_key->public_key_header_length + 11]) << 8
            | (trustm_rsa_key->public_key[trustm_rsa_key->public_key_header_length + 12]
            );  // get the total length in bytes of modulus

        memcpy(
            modulus_buffer,
            (trustm_rsa_key->public_key + trustm_rsa_key->public_key_header_length + 13),
            modulus_length
        );
    }
...
static int trustm_genpkey_ec(trustm_object_ctx_t *trustm_object_ctx) {
    ...
    uint8_t eccheader384[] = {0x30, 0x76,  // SEQUENCE
                              ...
                              0x02, 0x01, 0x06, 0x05,  // OID:1.3.132.0.34
    ...
    if (OPTIGA_LIB_SUCCESS != return_status) {
        TRUSTM_PROVIDER_ERRFN(
            "Error in optiga_crypt_ecc_generate_keypair\nError code : 0x%.4X\n",
            return_status
        );
        goto error;
    }
    ...
    uint16_t public_id = ((trustm_object_ctx->key_curve == OPTIGA_ECC_CURVE_NIST_P_521)
                          || (trustm_object_ctx->key_curve == OPTIGA_ECC_CURVE_BRAIN_POOL_P_512R1))
                             ? (trustm_object_ctx->key_id + 0x10EF)
                             : (trustm_object_ctx->key_id + 0x10E0);
...
static int trustm_object_loadkey_e0e0(
    trustm_object_ctx_t *trustm_object_ctx,
    OSSL_CALLBACK *object_cb,
    void *object_cbarg
) {
...
static int trustm_object_load_pkey_ec(
    trustm_object_ctx_t *trustm_object_ctx,
    OSSL_CALLBACK *object_cb,
    void *object_cbarg
) {
    ...
    if (trustm_ec_key->key_curve == OPTIGA_ECC_CURVE_NIST_P_521
    ...
    switch (trustm_ec_key->key_curve) {
        ...
        case OPTIGA_ECC_CURVE_BRAIN_POOL_P_256R1:
            if (memcmp(
                    ...
                    (void *)eccheaderBrainPool256,
                    sizeof(eccheaderBrainPool256)
                )) {
                TRUSTM_PROVIDER_ERRFN("Invalid public key. Please generate another key\n");
                goto error;
            }
        ...
        case OPTIGA_ECC_CURVE_BRAIN_POOL_P_384R1:
            if (memcmp(
                    (void *)read_data_buffer,
                    (void *)eccheaderBrainPool384,
                    sizeof(eccheaderBrainPool384)
                )) {
                TRUSTM_PROVIDER_ERRFN("Invalid public key. Please generate another key\n");
                goto error;
            }
        ...
        case OPTIGA_ECC_CURVE_BRAIN_POOL_P_512R1:
            if (memcmp(
                    (void *)read_data_buffer,
                    (void *)eccheaderBrainPool512,
                    sizeof(eccheaderBrainPool512)
                )) {
                TRUSTM_PROVIDER_ERRFN("Invalid public key header. Please generate another key\n");
                goto error;
            }
