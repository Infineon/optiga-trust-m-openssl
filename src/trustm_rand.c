...
static int trustm_rand_instantiate(
    void *ctx,
...
static int trustm_rand_uninstantiate(void *ctx) {
    return 1;
}

static int trustm_rand_generate(
    void *ctx,
    unsigned char *out,
    size_t outlen,
    unsigned int strength,
...
static int trustm_rand_enable_locking(void *ctx) {
    trustm_rand_ctx_t *trustm_rand_ctx = ctx;
    trustm_rand_ctx->lock = CRYPTO_THREAD_lock_new();
    return 1;
}

static int trustm_rand_lock(void *ctx) {
    trustm_rand_ctx_t *trustm_rand_ctx = ctx;
...
static void trustm_rand_unlock(void *ctx) {
    trustm_rand_ctx_t *trustm_rand_ctx = ctx;
...
static const OSSL_PARAM *trustm_rand_gettable_ctx_params(void *ctx, void *provctx) {
...
static int trustm_rand_get_ctx_params(void *ctx, OSSL_PARAM params[]) {
    OSSL_PARAM *p;
    TRUSTM_PROVIDER_DBGFN(">");
